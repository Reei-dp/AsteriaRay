...
// Protector is implemented on Android to call [android.net.VpnService.protect] for outbound sockets.
// Required when using a full-tunnel TUN: otherwise uplink dials can loop back into the VPN.
type Protector interface {
	ProtectFd(fd int32) bool
}

var (
	vpnProtector        Protector
	protectorRegisterOnce sync.Once
)

// RegisterProtector registers socket protection with Xray's system dialer. Call from [VpnService]
// before [CoreController.StartLoop] (after [VpnService.Builder.establish]).
func RegisterProtector(p Protector) {
	vpnProtector = p
	protectorRegisterOnce.Do(func() {
		if err := internet.RegisterDialerController(func(network, address string, c syscall.RawConn) error {
			return c.Control(func(fd uintptr) {
				if vpnProtector != nil {
					vpnProtector.ProtectFd(int32(fd))
				}
			})
		}); err != nil {
			log.Printf("RegisterDialerController: %v", err)
		}
	})
}
