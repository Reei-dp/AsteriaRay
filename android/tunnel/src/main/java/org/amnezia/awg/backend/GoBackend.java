...
public final class GoBackend implements Backend {
    ...
    /**
     * Host app may set this to a subclass of {@link GoBackend.VpnService} that calls
     * {@code startForeground} (e.g. persistent tray notification). Default is the inner {@link VpnService}.
     */
    public static Class<?> vpnServiceClass = VpnService.class;
    ...
    private void launchStatusJob() {
        ...
        statusThread = new Thread(() -> {
            // Defer first IpcGet until after initial handshake traffic; concurrent JNI IpcGet
            // during handshake initiation has been observed to crash libwg-go on some devices.
            try {
                Thread.sleep(4000);
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state)
        ...
        if (state == State.UP) {
            ...
            if (!vpnService.isDone()) {
                ...
                final Class<?> cls = vpnServiceClass != null ? vpnServiceClass : VpnService.class;
                final Intent vpnIntent = new Intent(context, cls);
