...
// Upstream tray_manager leaves this unimplemented on Linux: the shell is supposed to
// show the DBus-exported menu on right-click, but on many setups it never appears.
// Popup the same GtkMenu near the Flutter window so Dart can open it (shortcut / future hooks).
static FlMethodResponse* pop_up_context_menu(TrayManagerPlugin* self,
  ...
  GtkWidget* anchor = GTK_WIDGET(view);
  gtk_menu_popup_at_widget(GTK_MENU(menu), anchor, GDK_GRAVITY_NORTH_EAST,
